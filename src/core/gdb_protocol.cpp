...
struct GDBMemoryRegion {
  PhysicalMemoryAddress base;
  u32 length;
  u8* ptr;

  bool contains(PhysicalMemoryAddress address, u32 size) const {
    return (address >= base) && (address + size <= base + length);
  }
  u8* data(PhysicalMemoryAddress address) const {
    return ptr + (address - base);
  }
};

static std::vector<GDBMemoryRegion> GetMemoryRegions() {
  return {
    GDBMemoryRegion { Bus::RAM_BASE, Bus::RAM_SIZE, Bus::g_ram },
    GDBMemoryRegion { CPU::DCACHE_LOCATION, CPU::DCACHE_SIZE, CPU::g_state.dcache.data() },
    GDBMemoryRegion { Bus::BIOS_BASE, Bus::BIOS_SIZE, Bus::g_bios },
  };
}
...
static std::optional<std::string> Cmd$m(const std::string_view& data)
{
  ...
  if (address && length) {
    ...
    for (const auto& region : GetMemoryRegions()) {
      if (region.contains(phys_addr, phys_length)) {
        return { StringUtil::EncodeHex(region.data(phys_addr), phys_length) };
      }
    }
  }
  return { "E00" };
...
static std::optional<std::string> Cmd$M(const std::string_view& data)
{
  ...
  if (address && length && payload && (payload->size() == *length)) {
    ...
    for (const auto& region : GetMemoryRegions()) {
      if (region.contains(phys_addr, phys_length)) {
        memcpy(region.data(phys_addr), payload->data(), phys_length);
        return { "OK" };
      }
    }
  }
